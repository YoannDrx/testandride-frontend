...
import { View, Text, StyleSheet, Button, Image, TouchableOpacity, ScrollView, KeyboardAvoidingView, SafeAreaView, TextInput } from "react-native";
...
export default function SignUpForm(props) {
    ...
    let ctrlValidEmail = email === initalFormState.email || email.match(new RegExp(/^[A-Za-z0-9._-]+@testandride\.fr$/));
    let ctrlValidPassword = password === initalFormState.password || password.length >= 8;
    let ctrlValidPhoneNumber = tels[0].num === initalFormState.tels[0].num || tels[0].num.match(new RegExp(/^(?:(?:\+|00)33|0)\s*[1-9](?:[\s.-]*\d{2}){4}$/));
    let wholeFormComplete =
        checkBody(formValues, ["firstName", "lastName", "password", "passwordConf", "email", "emailConf"]) &&
        tels[0].num !== "" &&
        ctrlSamePassword &&
        ctrlSameEmail &&
        ctrlValidEmail &&
    ...
    return (
        <View style={styles.modalContainer}>
            <View style={styles.header}>
                ...
                <TouchableOpacity onPress={() => props.toggleModalSignUP()} style={styles.closeButton}>
                    <FontAwesome name={"times-circle-o"} style={styles.closeIcon} size={30}/>
                </TouchableOpacity>
            </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false} style={styles.scrollView}>
                <KeyboardAvoidingView style={styles.keyBoardAvoidingView} behavior="padding">
                    ...
                    <View style={ctrlValidEmail ? styles.inputCont : styles.inputContDanger}>
                        <TextInput
                            style={styles.input}
                            ref={upEmailRef}
                            placeholder="Email"
                    ...
                    <View style={ctrlSamePassword ? styles.inputCont : styles.inputContDanger}>
                        <TextInput
                            style={styles.input}
